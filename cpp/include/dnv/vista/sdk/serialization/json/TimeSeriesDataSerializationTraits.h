    ...
    public:
                    ...
                    if( auto timeRangeOpt = header.timeRange() )
                    {
                        ...
                        if( auto tsRef = timeSpanDoc.rootRef<Object>() )
                        {
                            ...
                            tsObj.emplace_back(
                                "Start",
                                nfx::json::Document{
                                    timeRange.start().toString( DateTime::Format::Iso8601PreciseTrimmed ) } );
                            tsObj.emplace_back(
                                "End",
                                nfx::json::Document{
                                    timeRange.end().toString( DateTime::Format::Iso8601PreciseTrimmed ) } );
                        }
        ...
        static void deserialize( const SerializableDocument& doc, timeseries::TimeSeriesDataPackage& obj )
        {
            ...
            if( headerDocRef.isValid() )
            {
                ...
                header = timeseries::Header{ *shipIdOpt, timeRange,           dateCreated,  dateModified,
                                             author,     systemConfiguration, customHeaders };
            }
