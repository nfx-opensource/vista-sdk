		...
		static void serialize( const dnv::vista::sdk::transport::timeseries::TimeSeriesDataPackage& obj, Document& doc )
		{
			...
			if ( package.header().has_value() )
			{
				...
				if ( auto timeRangeOpt = header.timeRange() )
				{
					...
					doc.set( headerPath + "/TimeSpan/Start", timeRange.start().toString( DateTime::Format::Iso8601PreciseTrimmed ) );
					doc.set( headerPath + "/TimeSpan/End", timeRange.end().toString( DateTime::Format::Iso8601PreciseTrimmed ) );
				}

				// DateCreated (optional)
				if ( auto dateCreated = header.dateCreated() )
				{
					doc.set( headerPath + "/DateCreated", dateCreated->toString( DateTime::Format::Iso8601PreciseTrimmed ) );
				}

				// DateModified (optional)
				if ( auto dateModified = header.dateModified() )
				{
					doc.set( headerPath + "/DateModified", dateModified->toString( DateTime::Format::Iso8601PreciseTrimmed ) );
				}
				...
				if ( auto sysConfigsOpt = header.systemConfiguration() )
				{
					...
					for ( std::size_t i = 0; i < sysConfigs.size(); ++i )
					{
						...
						doc.set( configPath + "/TimeStamp", config.timeStamp().toString( DateTime::Format::Iso8601PreciseTrimmed ) );
					}
				}
			...
			for ( std::size_t tsIdx = 0; tsIdx < timeSeriesData.size(); ++tsIdx )
			{
				...
				if ( tsData.dataConfiguration().has_value() )
				{
					...
					doc.set( tsPath + "/DataConfiguration/TimeStamp", dataConfig.timeStamp().toString( DateTime::Format::Iso8601PreciseTrimmed ) );
				}
				...
				if ( auto tabularDataListOpt = tsData.tabularData() )
				{
					...
					for ( std::size_t tabIdx = 0; tabIdx < tabularDataList.size(); ++tabIdx )
					{
						...
						for ( std::size_t dsIdx = 0; dsIdx < dataSets.size(); ++dsIdx )
						{
							...
							doc.set( dsPath + "/TimeStamp", dataSet.timeStamp().toString( DateTime::Format::Iso8601PreciseTrimmed ) );
				...
				if ( tsData.eventData().has_value() )
				{
					...
					if ( eventData.dataSet().has_value() )
					{
						...
						for ( std::size_t edIdx = 0; edIdx < eventDataSets.size(); ++edIdx )
						{
							...
							doc.set( edPath + "/TimeStamp", eventDataSet.timeStamp().toString( DateTime::Format::Iso8601PreciseTrimmed ) );
				...
				if ( doc.contains( tsPath + "/TabularData" ) )
				{
					...
					while ( true )
					{
						...
						while ( true )
						{
							...
							{
								throw std::runtime_error{ "Invalid ISO 19848 DateTimeOffset format in TabularDataSet TimeStamp: " + *timestampStr };
							}
