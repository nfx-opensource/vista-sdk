...
namespace dnv::vista::sdk::transport::datachannel
{
	...
	inline const std::optional<nfx::serialization::json::Document>& Header::customHeaders() const noexcept
	{
	...
	inline void Header::setCustomHeaders( std::optional<nfx::serialization::json::Document> customHeaders )
	{
	...
	inline std::optional<std::reference_wrapper<const DataChannel>> DataChannelList::from( std::string_view shortId ) const noexcept
	{
		auto it = std::find_if( m_dataChannels.begin(), m_dataChannels.end(),
			[&shortId]( const DataChannel& dc ) {
				return dc.dataChannelId().shortId().has_value() &&
					   dc.dataChannelId().shortId().value() == shortId;
			} );
	...
	inline std::optional<std::reference_wrapper<const DataChannel>> DataChannelList::from( const LocalId& localId ) const noexcept
	{
		auto it = std::find_if( m_dataChannels.begin(), m_dataChannels.end(),
			[&localId]( const DataChannel& dc ) {
	...
	inline void DataChannelList::add( const std::vector<DataChannel>& dataChannels )
	{
		...
		for ( const auto& dc : dataChannels )
		{
			...
			auto localIdExists = std::any_of( m_dataChannels.begin(), m_dataChannels.end(),
				[&dc]( const DataChannel& existing ) {
			...
			if ( dc.dataChannelId().shortId().has_value() )
			{
				...
				auto shortIdExists = std::any_of( m_dataChannels.begin(), m_dataChannels.end(),
					[&shortId]( const DataChannel& existing ) {
