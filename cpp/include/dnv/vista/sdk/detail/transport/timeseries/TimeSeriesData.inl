...
namespace dnv::vista::sdk::transport::timeseries
{
	...
	inline ConfigurationReference::ConfigurationReference( std::string id, DateTimeOffset timeStamp )
		: m_id{ std::move( id ) }, m_timeStamp{ std::move( timeStamp ) }
	{
	}
	...
	inline TimeRange::TimeRange( DateTimeOffset start, DateTimeOffset end )
		: m_start{ std::move( start ) }, m_end{ std::move( end ) }
	{
		if ( m_start > m_end )
		{
			throw std::invalid_argument{ "TimeRange start must be before or equal to end" };
		}
	}

	inline const DateTimeOffset& TimeRange::start() const noexcept
	{
		return m_start;
	}

	inline const DateTimeOffset& TimeRange::end() const noexcept
	{
		return m_end;
	}

	inline void TimeRange::setStart( DateTimeOffset start )
	{
		if ( start > m_end )
		{
			throw std::invalid_argument{ "TimeRange start must be before or equal to end" };
	...
	inline void TimeRange::setEnd( DateTimeOffset end )
	{
		if ( m_start > end )
		{
			throw std::invalid_argument{ "TimeRange start must be before or equal to end" };
	...
	inline TabularDataSet::TabularDataSet(
		...
		: m_timeStamp{ std::move( timeStamp ) }, m_value{ std::move( value ) }, m_quality{ std::move( quality ) }
	{
	}
	...
	inline TabularData::TabularData(
		...
		: m_dataChannelIds{ std::move( dataChannelIds ) }, m_dataSets{ std::move( dataSets ) }
	{
	}
		...
		for ( std::size_t i = 0; i < m_dataSets.size(); ++i )
		{
			...
			if ( dataset.value().size() != m_dataChannelIds.size() )
			{
				return ValidateResult<>::invalid( { "Tabular data set " + std::to_string( i ) + " expects " +
													...
													std::to_string( dataset.value().size() ) + " values are provided" } );
			}
	...
	inline EventDataSet::EventDataSet(
		...
		: m_timeStamp{ std::move( timeStamp ) }, m_dataChannelId{ std::move( dataChannelId ) }, m_value{ std::move( value ) }, m_quality{ std::move( quality ) }
	{
	}
	...
	inline TimeSeriesData::TimeSeriesData(
		...
		: m_dataConfiguration{ std::move( dataConfiguration ) }, m_tabularData{ std::move( tabularData ) }, m_eventData{ std::move( eventData ) }, m_customDataKinds{ std::move( customDataKinds ) }
	{
	}
	...
	inline Package::Package(
		...
		: m_header{ std::move( header ) }, m_timeSeriesData{ std::move( timeSeriesData ) }
	{
	}
