...
namespace dnv::vista::sdk::transport
{
	...
	inline auto Value::match( FDecimal&& onDecimal, FInteger&& onInteger, FBoolean&& onBoolean, FString&& onString, FDateTime&& onDateTime ) const
	{
		return std::visit(
	...
	template <typename FDecimal, typename FInteger, typename FBoolean, typename FString, typename FDateTime>
	inline void Value::visit( FDecimal&& onDecimal, FInteger&& onInteger, FBoolean&& onBoolean, FString&& onString, FDateTime&& onDateTime ) const
	{
		std::visit(
			[&]( auto&& value ) {
				using T = std::decay_t<decltype( value )>;
	...
	template <typename FDecimal, typename FInteger, typename FBoolean, typename FString, typename FDateTime>
	inline auto FormatDataType::match( std::string_view value, FDecimal&& onDecimal, FInteger&& onInteger, FBoolean&& onBoolean, FString&& onString, FDateTime&& onDateTime ) const
	{
		auto result = validate( value );
		if ( !result.isOk() || !result.value )
			throw std::invalid_argument{ "Validation failed: " +
										 ( result.errors.empty() ? std::string{ "unknown error" }
																 : result.errors[0] ) };
