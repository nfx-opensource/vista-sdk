...
namespace dnv::vista::sdk
{
	...
	protected:
		...
		[[nodiscard]] static bool matchFilterAgainstTarget(
			const std::unordered_map<std::string, NodeItem>& filter,
	...
	public:
		 ...
		 */
		[[nodiscard]] Path withNode( std::function<const GmodNode*( const std::unordered_map<std::string, const GmodNode*>& )> select,
		 ...
		 */
		[[nodiscard]] Path withAnyNodeBefore( std::function<const GmodNode*( const std::unordered_map<std::string, const GmodNode*>& )> select ) const&;
		 ...
		 * @throws std::invalid_argument if node not in path
		 */
		[[nodiscard]] Path withAnyNodeBefore( std::function<const GmodNode*( const std::unordered_map<std::string, const GmodNode*>& )> select ) &&;
