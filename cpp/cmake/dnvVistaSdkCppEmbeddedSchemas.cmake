...
add_library(dnv-vista-cpp-embeddedschemas
	OBJECT
...
set_target_properties(dnv-vista-cpp-embeddedschemas
	PROPERTIES
		CXX_STANDARD 20
...
set(DNV_VISTA_SDK_CPP_EMBEDDED_SCHEMAS_OUTPUT_DIR "${CMAKE_BINARY_DIR}/generated/dnv-vista-cpp-schemas-registry")
...
function(embed_all_schemas_to_target TARGET_NAME SCHEMAS_DIR OUTPUT_DIR)
	...
	foreach(SCHEMA_FILE ${SCHEMA_FILES})
		...
		add_custom_command(
			OUTPUT "${OUTPUT_CPP}"
			COMMAND $<TARGET_FILE:dnv-vista-cpp-resourcegenerator> "${SCHEMA_FILE}" "${OUTPUT_CPP}" "${SCHEMA_ID}" "${REL_PATH}" "dnv::vista::sdk::schemas"
			DEPENDS "${SCHEMA_FILE}" dnv-vista-cpp-resourcegenerator
	...
	foreach(SCHEMA_ID ${ALL_SCHEMA_IDS})
		...
		string(APPEND REGISTRY_ENTRIES "        { ${SCHEMA_ID}_name, ${SCHEMA_ID}_data, ${SCHEMA_ID}_size },\n")
	endforeach()
...
embed_all_schemas_to_target(
	...
	"${DNV_VISTA_SDK_CPP_SCHEMAS_DIR}"
	"${DNV_VISTA_SDK_CPP_EMBEDDED_SCHEMAS_OUTPUT_DIR}"
)
