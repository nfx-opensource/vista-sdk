...
namespace dnv::vista::sdk::tests
{
	...
	transport::timeseries::TimeSeriesDataPackage createTestTimeSeriesDataPackage()
	{
		...
		nfx::serialization::json::Document customDataKinds1;
		...
		nfx::serialization::json::Document customDataKinds2;
	...
	TEST( TimeSeriesTests, Test_EventData_Operations )
	{
		...
		dataSets.push_back( transport::timeseries::EventDataSet{
			timestamp,
			*dataChannelIdOpt,
			"HIGH",
			std::string{ "0" } } );
	...
	TEST( CrossValidationTests, Invalid_DataChannelId_Not_Found )
	{
		...
		transport::timeseries::TimeSeriesData tsData{
			dataConfig,
			std::vector<transport::timeseries::TabularData>{ tabularData },
			std::nullopt,
			std::nullopt };
	...
	TEST( CrossValidationTests, Invalid_Value_Format_Mismatch )
	{
		...
		auto channelIdOpt = transport::timeseries::DataChannelId::fromString(
			firstChannel.dataChannelId().shortId().has_value() ? *firstChannel.dataChannelId().shortId() : firstChannel.dataChannelId().localId().toString() );
		...
		std::vector<transport::timeseries::TabularDataSet> dataSets{
			transport::timeseries::TabularDataSet(
				timestamp,
				std::vector<std::string>{ "invalid_not_a_number" },
				std::nullopt ) };
	...
	TEST( CrossValidationTests, Invalid_DataConfiguration_Id_Mismatch )
	{
		...
		transport::timeseries::ConfigurationReference wrongDataConfig{
			"WrongDataChannelList.xml",
			DateTimeOffset{ "2016-01-01T00:00:00Z" } };

		transport::timeseries::TimeSeriesData tsData{
			wrongDataConfig,
			originalTsData.tabularData(),
			originalTsData.eventData(),
			std::nullopt };
	...
	TEST( IsoMessageTests, TimeSeriesDataJson )
	{
		...
		std::string json = nfx::serialization::json::Serializer<transport::timeseries::TimeSeriesDataPackage>::toString( originalPackage, options );

		// Deserialize JSON to domain model
		auto docOpt = nfx::serialization::json::Document::fromString( json );
		ASSERT_TRUE( docOpt.has_value() );

		transport::timeseries::TimeSeriesDataPackage deserializedPackage{
			transport::timeseries::Package{
				std::nullopt,
				std::vector<transport::timeseries::TimeSeriesData>{} } };

		nfx::serialization::json::SerializationTraits<transport::timeseries::TimeSeriesDataPackage>::deserialize( *docOpt, deserializedPackage );
		...
		// TimeRange
		ASSERT_TRUE( originalHeader->timeRange().has_value() );
		ASSERT_TRUE( deserializedHeader->timeRange().has_value() );
		EXPECT_EQ( originalHeader->timeRange()->start().toString(),
			deserializedHeader->timeRange()->start().toString() );
		EXPECT_EQ( originalHeader->timeRange()->end().toString(),
			deserializedHeader->timeRange()->end().toString() );

		// DateCreated
		ASSERT_TRUE( originalHeader->dateCreated().has_value() );
		ASSERT_TRUE( deserializedHeader->dateCreated().has_value() );
		EXPECT_EQ( originalHeader->dateCreated()->toString(),
			deserializedHeader->dateCreated()->toString() );

		// DateModified
		ASSERT_TRUE( originalHeader->dateModified().has_value() );
		ASSERT_TRUE( deserializedHeader->dateModified().has_value() );
		EXPECT_EQ( originalHeader->dateModified()->toString(),
			deserializedHeader->dateModified()->toString() );
		...
		for ( std::size_t i = 0; i < originalSysConfigs.size(); ++i )
		{
			...
			EXPECT_EQ( originalSysConfigs[i].timeStamp().toString(),
				deserializedSysConfigs[i].timeStamp().toString() );
		}
		...
		for ( std::size_t tsIdx = 0; tsIdx < originalTimeSeriesData.size(); ++tsIdx )
		{
			...
			EXPECT_EQ( originalTsData.dataConfiguration()->id(),
				deserializedTsData.dataConfiguration()->id() );
			EXPECT_EQ( originalTsData.dataConfiguration()->timeStamp().toString(),
			...
			for ( std::size_t tabIdx = 0; tabIdx < originalTabularDataList.size(); ++tabIdx )
			{
				...
				{
					EXPECT_EQ( originalChannelIds[cidIdx].toString(),
						deserializedChannelIds[cidIdx].toString() );
				}
				...
				for ( std::size_t dsIdx = 0; dsIdx < originalDataSets.size(); ++dsIdx )
				{
					...
					// TimeStamp
					EXPECT_EQ( originalDataSet.timeStamp().toString(),
						deserializedDataSet.timeStamp().toString() );
			...
			if ( originalEventData.dataSet().has_value() )
			{
				...
				for ( std::size_t edIdx = 0; edIdx < originalEventDataSets.size(); ++edIdx )
				{
					...
					// TimeStamp
					EXPECT_EQ( originalEventDataSet.timeStamp().toString(),
						deserializedEventDataSet.timeStamp().toString() );

					// DataChannelId
					EXPECT_EQ( originalEventDataSet.dataChannelId().toString(),
