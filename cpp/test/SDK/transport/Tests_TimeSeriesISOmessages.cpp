...
namespace dnv::vista::sdk::tests
{
    ...
    TEST( IsoMessageTests, TimeSeriesDataJson )
    {
        ...
        transport::timeseries::TimeSeriesDataPackage deserializedPackage{ transport::timeseries::Package{
            std::nullopt, std::vector<transport::timeseries::TimeSeriesData>{} } };

        nfx::serialization::json::SerializationTraits<transport::timeseries::TimeSeriesDataPackage>::deserialize(
        ...
        EXPECT_EQ(
            originalHeader->timeRange()->start().toString(), deserializedHeader->timeRange()->start().toString() );
        EXPECT_EQ( originalHeader->timeRange()->end().toString(), deserializedHeader->timeRange()->end().toString() );
