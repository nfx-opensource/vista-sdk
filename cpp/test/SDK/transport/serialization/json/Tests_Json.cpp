...
namespace dnv::vista::sdk::tests
{
	...
	{
		auto docOpt = Document::fromString( jsonStr );
		if ( !docOpt.has_value() )
		{
		...
		SerializationTraits<DataChannelListPackage>::deserialize( dataChannelListPackage, doc );
	...
	std::string serializeTimeSeriesData( const transport::timeseries::TimeSeriesDataPackage& package )
	{
		...
		options.includeNullFields = false;
		options.prettyPrint = true;
		return nfx::serialization::json::Serializer<transport::timeseries::TimeSeriesDataPackage>::toString( package, options );
	}
	...
	TEST( DataChannelListJsonTests, ISO8601_DateTimeOffset )
	{
		...
		for ( const auto& format : validFormats )
		{
			...
			EXPECT_NO_THROW( {
				...
				EXPECT_EQ( package.package().header().dataChannelListId().timeStamp().ticks(),
		...
		{
			auto json = createTestJson( format );
			EXPECT_THROW( { auto package = deserializeDataChannelList( json ); }, std::runtime_error ) << "Should reject invalid format: " << format;
		}
	}
	...
	TEST( TimeSeriesDataTests, ISO8601_DateTimeOffset )
	{
		...
		for ( const auto& format : validFormats )
		{
			...
			EXPECT_NO_THROW( {
				...
				<< "Should parse valid format: " << format;
		}
		...
		{
			auto json = createTestJson( format );
			EXPECT_THROW( { auto package = deserializeTimeSeriesData( json ); }, std::runtime_error ) << "Should reject invalid format: " << format;
		}
	}
	...
	TEST( DataChannelListJsonTests, DataChannelList_JSONSchema_Validation )
	{
		...
		for ( const auto& fileName : testFiles )
		{
			...
			auto docOpt = Document::fromString( *jsonDataOpt );
			...
			EXPECT_TRUE( package.package().dataChannelList().size() > 0 )
				<< "Should have channels in " << fileName;
		}
	}
	...
	TEST( DataChannelListJsonTests, DataChannelList_File_Serialization )
	{
		...
		for ( const auto& fileName : testFiles )
		{
			...
			EXPECT_EQ( package.package().dataChannelList().size(),
				roundtrippedPackage.package().dataChannelList().size() )
	...
	TEST( TimeSeriesDataTests, TimeSeriesData_File_Serialization )
	{
		...
		EXPECT_EQ( package.package().timeSeriesData().size(),
			roundtrippedPackage.package().timeSeriesData().size() )
	...
	TEST( DataChannelListJsonTests, DataChannelList_Serialization_Roundtrip )
	{
		...
		EXPECT_EQ( package.package().header().shipId().toString(), deserializedPackage.package().header().shipId().toString() );
		EXPECT_EQ( package.package().header().dataChannelListId().id(), deserializedPackage.package().header().dataChannelListId().id() );
		EXPECT_EQ( package.package().header().dataChannelListId().timeStamp().ticks(),
		...
		EXPECT_EQ( channel1.dataChannelId().localId().toString(), deserializedChannel1.dataChannelId().localId().toString() );
		EXPECT_EQ( channel1.dataChannelId().shortId(), deserializedChannel1.dataChannelId().shortId() );
		EXPECT_EQ( channel1.property().dataChannelType().type(), deserializedChannel1.property().dataChannelType().type() );
	}
	...
	TEST( TimeSeriesDataTests, TimeSeriesData_Serialization_Roundtrip )
	{
		...
		if ( package.package().header().has_value() )
		{
			...
			EXPECT_EQ( package.package().header()->shipId().toString(),
	...
	TEST( TimeSeriesDataTests, TimeSeriesData_DateTimeOffset_Consistency )
	{
			 ...
			 package.package().timeSeriesData()[0].dataConfiguration().has_value() )
		{
			...
			auto deserializedTimeStamp = deserializedPackage.package().timeSeriesData()[0].dataConfiguration()->timeStamp();
		...
		// Also verify DateCreated
		if ( package.package().header().has_value() &&
			 package.package().header()->dateCreated().has_value() )
		{
