...
namespace dnv::vista::sdk::tests
{
	...
	transport::datachannel::DataChannelListPackage createValidFullyCustomDataChannelList()
	{
		...
		// std::cout << customHeaders.toString( 4 ) << std::endl;

		transport::datachannel::Header header{
			dnv::vista::sdk::ShipId::fromString( "IMO1234567" ).value(),
			dataChannelListId,
			versionInfo,
			std::string{ "Author1" },
	...
	transport::datachannel::DataChannelListPackage createValidDataChannelList()
	{
		...
		// Create Header
		transport::datachannel::Header header{
			dnv::vista::sdk::ShipId::fromString( "IMO1234567" ).value(), // ShipId
	...
	TEST( IsoMessageTests, DataChannelListJsonSerializationComplete )
	{
		...
		std::string json = nfx::serialization::json::Serializer<transport::datachannel::DataChannelListPackage>::toString( originalPackage, options );
		...
		transport::datachannel::DataChannelListPackage deserializedPackage{ transport::datachannel::Package{
			transport::datachannel::Header{ ShipId::fromString( "IMO1234567" ).value(),
				transport::datachannel::ConfigurationReference{
					"dummy",
					DateTimeOffset{},
					std::nullopt } },
			transport::datachannel::DataChannelList{} } };

		nfx::serialization::json::SerializationTraits<transport::datachannel::DataChannelListPackage>::deserialize( deserializedPackage, *docOpt );
		...
		EXPECT_EQ( originalHeader.dataChannelListId().timeStamp().toString(),
		...
		EXPECT_EQ( originalHeader.versionInformation()->namingRule(),
			deserializedHeader.versionInformation()->namingRule() );
		EXPECT_EQ( originalHeader.versionInformation()->namingSchemeVersion(),
			deserializedHeader.versionInformation()->namingSchemeVersion() );
		EXPECT_EQ( originalHeader.versionInformation()->referenceUrl(),
		...
		// DataChannelId
		EXPECT_EQ( originalChannel1.dataChannelId().localId().toString(),
		...
		auto deserializedObjOpt = deserializedCustomNameObjects.get<nfx::serialization::json::Document::Object>( "" );
		...
		EXPECT_EQ( *originalProperty1.dataChannelType().updateCycle(), *deserializedProperty1.dataChannelType().updateCycle() );
		...
		EXPECT_EQ( deserializedCustomElements.get<std::string>( "nr:CustomUnitElement" ), "Vendor specific unit element" );
		...
		EXPECT_EQ( deserializedCustomProperties.get<std::string>( "nr:CustomPropertyElement" ), "Vendor specific property element" );
		...
		EXPECT_EQ( deserializedCustomProperties.get<std::string>( "installationDate" ), "2023-06-15T08:00:00.0000000Z" );
		...
		// DataChannelId
		EXPECT_EQ( originalChannel2.dataChannelId().localId().toString(),
