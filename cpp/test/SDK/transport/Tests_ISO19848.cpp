	...
	TEST_F( ISO19848Tests, Match_Decimal_Return_Values )
	{
		...
		ASSERT_NO_THROW( {
			result = decimalType.match(
				"123.456", []( const Decimal& d ) -> std::string { return "decimal:" + d.toString(); },
				[]( int ) -> std::string {
					unexpectedInteger();
					return "";
				},
		...
		ASSERT_NO_THROW( {
			doubleResult = decimalType.match(
				"987.654", []( const Decimal& d ) -> double { return d.toDouble(); },
				[]( int ) -> double {
					unexpectedInteger();
					return 0.0;
				},
	...
	TEST_F( ISO19848Tests, Match_Edge_Cases_And_Return_Types )
	{
		...
		ASSERT_NO_THROW( {
			info = decimalType.match(
				"-456.789", []( const Decimal& d ) -> DecimalInfo { return { d, d < Decimal{ 0 } }; },
				[]( int ) -> DecimalInfo {
					unexpectedInteger();
					return {};
				},
