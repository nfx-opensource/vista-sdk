...
namespace dnv::vista::sdk::test
{
		...
		::testing::Values(
			MatchTestData{
				...
				[]() {
					...
					return LocalIdQueryBuilder::empty()
						.withPrimaryItem( *path, []( GmodPathQueryBuilder::Path builder ) {
			...
			MatchTestData{
				...
				[]() {
					...
					return LocalIdQueryBuilder::empty()
						.withSecondaryItem( *path, []( GmodPathQueryBuilder::Path builder ) {
							return builder.withoutLocations().build();
						} )
						.build();
				},
			...
			MatchTestData{
				...
				[]() {
					...
					return LocalIdQueryBuilder::from( *localId )
						.withPrimaryItem( localId->primaryItem(), [&node433]( GmodPathQueryBuilder::Path builder ) {
							return builder.withNode( [&node433]( const std::unordered_map<std::string, const GmodNode*>& nodes ) {
											  return *node433;
										  },
											  true )
								.build();
	...
	TEST_F( LocalIdQueryTests, WithAnyNodeBefore )
	{
		...
		auto generalQuery = LocalIdQueryBuilder::empty()
	...
	TEST_F( LocalIdQueryTests, HappyPath )
	{
		...
		for ( bool individualized : { true, false } )
		{
			...
			query = builder.build();
			EXPECT_TRUE( query.match( *localId ) ) << "Failed to match primary (individualized=" << individualized << ")";

			// Match secondary
			builder = LocalIdQueryBuilder::empty().withSecondaryItem( secondaryQuery );
			query = builder.build();
			EXPECT_TRUE( query.match( *localId ) ) << "Failed to match secondary (individualized=" << individualized << ")";

			// Match tags
			builder = LocalIdQueryBuilder::empty();
			for ( const auto& tag : localId->metadataTags() )
			{
				builder = std::move( builder ).withTags( [&tag]( MetadataTagsQueryBuilder& tags ) {
					return tags.withTag( tag ).build();
				} );
			}
			query = builder.build();
			...
			query = builder.build();
			EXPECT_TRUE( query.match( *localId ) ) << "Failed to match primary+secondary (individualized=" << individualized << ")";
			...
			query = builder.build();
			EXPECT_TRUE( query.match( *localId ) ) << "Failed to match primary+tags (individualized=" << individualized << ")";
			...
			query = builder.build();
			EXPECT_TRUE( query.match( *localId ) ) << "Failed to match secondary+tags (individualized=" << individualized << ")";
