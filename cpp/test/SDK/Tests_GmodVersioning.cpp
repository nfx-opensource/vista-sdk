...
namespace dnv::vista::sdk::test
{
	...
	TEST_P( ConvertPath, ConvertPath )
	{
		...
		auto parsedTargetPath = GmodPath::fromShortPath( params.expectedPath, targetGmod, targetLocations, targetErrors );
	...
	TEST_P( ConvertFullPath, ConvertFullPath )
	{
		...
		auto parsedTargetPath = GmodPath::fromFullPath( params.expectedPath, targetGmod, targetLocations, targetErrors );
		ASSERT_TRUE( parsedTargetPath.has_value() ) << "Failed to parse expected path: " << params.expectedPath
													<< "\nErrors: " << targetErrors.toString();
	...
	INSTANTIATE_TEST_SUITE_P( GmodVersioningTests, ValidPathToLatest,
	...
	TEST( GmodVersioningTests, ConvertGmodPathWithLocation )
	{
		...
		auto expectedPath = GmodPath::fromShortPath( "691.83111i-A/H101.11-1", targetGmod, targetLocations, targetErrors );
