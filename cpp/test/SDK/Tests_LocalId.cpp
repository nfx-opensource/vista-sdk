...
namespace dnv::vista::sdk::test
{
    ...
    static std::vector<LocalIdTestCase> validTestData()
    {
        return {
            ...
            { LocalIdInput{
                  "411.1/C101.63/S206", std::nullopt, "temperature", "exhaust.gas", "inlet", VisVersion::v3_4a, true },
            ...
            { LocalIdInput{
                  "511.11/C101.67/S208", std::nullopt, "pressure", "starting.air", "inlet", VisVersion::v3_6a, true },
    ...
    {
        return { // Test case 1: Primary item with metadata tags
                 { LocalIdInput{ "411.1/C101.31-2", std::nullopt, "temperature", "exhaust.gas", "inlet" },
    ...
    TEST_P( LocalIdBuildValidTest, LocalId_Build_Valid )
    {
    ...
    INSTANTIATE_TEST_SUITE_P( LocalIdTests, LocalIdBuildValidTest, ::testing::ValuesIn( validTestData() ) );
    ...
    TEST_P( MqttLocalIdBuildValidTest, Mqtt_LocalId_Build_Valid )
    {
        auto testCase = GetParam();
        const auto& input = testCase.input;
    ...
    INSTANTIATE_TEST_SUITE_P( LocalIdTests, MqttLocalIdBuildValidTest, ::testing::ValuesIn( validMqttTestData() ) );

    //----------------------------------------------
    // Build with all without methods
    //----------------------------------------------

    TEST( LocalIdTests, LocalId_Build_AllWithout )
    {
        const auto& vis = VIS::instance();
    ...
    TEST_P( LocalIdEqualityTest, LocalId_Equality )
    {
        auto testCase = GetParam();
        const auto& input = testCase.input;
        ...
        if( input.secondaryItem.has_value() )
        {
            secondaryItem = GmodPath::fromShortPath( *input.secondaryItem, gmod, vis.locations( visVersion ), errors );
            ASSERT_TRUE( secondaryItem.has_value() );
        }
    ...
    INSTANTIATE_TEST_SUITE_P( LocalIdTests, LocalIdEqualityTest, ::testing::ValuesIn( validTestData() ) );
    ...
    TEST_P( LocalIdParsingTest, Parsing )
    {
    ...
    INSTANTIATE_TEST_SUITE_P( LocalIdTests, LocalIdParsingTest, ::testing::ValuesIn( validParsingData() ) );
