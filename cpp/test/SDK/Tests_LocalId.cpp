...
namespace dnv::vista::sdk::test
{
	...
	TEST_P( LocalIdBuildValidTest, LocalId_Build_Valid )
	{
		...
		auto primaryItem = GmodPath::fromShortPath( input.primaryItem, gmod, vis.locations( input.visVersion ), primaryErrors );
		...
		if ( input.secondaryItem.has_value() )
		{
			...
			secondaryItem = GmodPath::fromShortPath( *input.secondaryItem, gmod, vis.locations( input.visVersion ), secondaryErrors );
