...
namespace dnv::vista::sdk::test
{
	...
	{
	public:
		explicit ThreadSafeQueue( size_t maxSize = 10000 ) : m_maxSize( maxSize ) {}
	...
	TEST( GmodVersioningSmokeTests, VersioningTest )
	{
		...
		auto producer = std::thread( [&]() {
			auto completed = sourceGmod.traverse( [&]( const std::vector<const GmodNode*>& parents, const GmodNode& node ) {
				if ( parents.empty() )
				...
				while ( auto sourcePathOpt = queue.waitAndPop() )
				{
					...
					try
					{
						...
						if ( targetPath.has_value() )
						{
							...
							auto parsedPath = GmodPath::fromShortPath( targetPath->toString(), targetGmod, locations, errors );
					...
					{
						count = ++counter;
						std::cout << "[" << std::setw( pad ) << count << "]["
								  << std::setw( 2 ) << threadId << "] Failed to create valid path from: "
								  << sourcePath.toString() << " -> "
					...
					{
						std::cout << "[" << std::setw( pad ) << count << "]["
								  << std::setw( 2 ) << threadId << "] Paths processed\n";
					}
				}
			} );
		}
		...
		std::cout << "Success/failed - " << success << "/" << failed
				  << " (" << std::fixed << std::setprecision( 2 ) << successRate << "% success)\n";
		...
		std::sort( sortedExceptions.begin(), sortedExceptions.end(),
