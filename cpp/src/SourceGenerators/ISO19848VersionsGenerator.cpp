 ...
 * @details Generates ISO19848Versions.h (public API) and ISO19848VersionsExtensions.h (internal utilities) from embedded ISO 19848 resource files
 */
...
static bool generateISO19848VersionHeader( const std::vector<std::string>& versions, const std::filesystem::path& outputPath, std::string_view sdkVersion )
{
	...
	std::format_to( std::back_inserter( sb ), "Generated by: ISO19848VersionsGenerator (vista-sdk-cpp v{})\n", sdkVersion );
	sb.append( " */\n" );
	sb.append( "\n" );
	...
	sb.append( "\t\t */\n" );
	sb.append( "\t\t[[nodiscard]] constexpr std::optional<ISO19848Version> fromString( std::string_view version ) noexcept\n" );
	sb.append( "\t\t{\n" );
	for ( const auto& version : versions )
	{
...
static bool generateISO19848VersionExtensionsHeader( const std::vector<std::string>& versions, const std::filesystem::path& outputPath, std::string_view sdkVersion )
{
	auto sb = nfx::string::StringBuilder( 8192 );
	...
	sb.append( "\t\t */\n" );
	std::format_to( std::back_inserter( sb ), "\t\tconstexpr std::array<ISO19848Version, {}> all()\n", versions.size() );
	sb.append( "\t\t{\n" );
	sb.append( "\t\t\treturn {\n" );
...
int main( int argc, char* argv[] )
{
	if ( argc < 4 )
	{
		fprintf( stderr, "Usage: ISO19848VersionsGenerator <public_header_path> <extensions_header_path> <sdk_version>\n" );
		return 1;
	}
