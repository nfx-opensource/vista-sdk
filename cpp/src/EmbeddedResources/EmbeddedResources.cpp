...
namespace dnv::vista::sdk
{
	...
	std::vector<std::string> EmbeddedResources::iso19848Versions()
	{
		...
		for ( const auto& res : resources )
		{
			...
			// Filename pattern: "iso19848-{version}-data-channel-type-names.json.gz" e.g. "iso19848-v2018-data-channel-type-names.json.gz"
			if ( nfx::string::endsWith( res.name, ".gz" ) &&
				 nfx::string::contains( res.name, "iso19848" ) &&
		...
		if ( versions.empty() )
		{
			throw std::runtime_error{ "Did not find required resources (expected iso19848-*-data-channel-type-names.json.gz resources)" };
		}
