...
namespace dnv::vista::sdk
{
	...
	std::optional<Location> Locations::fromString(
		std::string_view locationStr,
		internal::LocationParsingErrorBuilder* errorBuilder ) const noexcept
	{
		...
		for ( std::size_t i = 0; i < locationStr.size(); ++i )
		{
			...
			if ( std::isdigit( static_cast<unsigned char>( ch ) ) )
			{
				...
				if ( prevDigitIndex.has_value() && prevDigitIndex.value() != ( i - 1 ) )
				{
					...
					{
						StringBuilder sb;
						sb << "Invalid location: cannot have multiple separated digits in location: '"
						   << locationStr
						   << '\'';
			...
			else
			{
					...
					if ( m_locationCodes.find( ch ) == m_locationCodes.end() )
					{
						if ( errorBuilder )
						{
							...
							errorMsg << "Invalid location code: '"
									 << locationStr
									 << "' with invalid location code(s): "
									 << invalidChars.toStringView();

							errorBuilder->addError( internal::LocationValidationResult::InvalidCode, errorMsg.toString() );
						}
				...
				if ( seenGroups[groupIndex].has_value() )
				{
					if ( errorBuilder )
					{
						...
						StringBuilder sb;
						sb << "Invalid location: Multiple '"
						   << groupName
						   << "' values. Got both '"
						   << seenGroups[groupIndex].value()
						   << "' and '"
						   << ch
						   << "' in '"
						   << locationStr
