...
namespace dnv::vista::sdk
{
	...
	std::vector<GmodIndividualizableSet> GmodPath::individualizableSets() const
	{
		...
		return std::vector<GmodIndividualizableSet>( result.begin(), result.end() );
	}
	...
	std::optional<std::string> GmodPath::normalAssignmentName( size_t nodeDepth ) const
	{
		...
		if ( normalAssignmentNames.isEmpty() )
		{
			return std::nullopt;
		}
		...
		for ( size_t i = length() - 1;; --i )
		{
			...
			if ( const auto* assignmentName = normalAssignmentNames.find( child.code() ) )
			{
	...
	std::vector<std::pair<size_t, std::string>> GmodPath::commonNames() const
	{
		...
		for ( const auto& [depth, node] : fullPath() )
		{
			...
			// Prefer commonName (generic name) if available, otherwise use name (specific name)
			// Note: Some nodes (e.g., installSubstructure nodes) may have empty/missing name but populated commonName
			std::string name;
			...
			if ( !normalAssignmentNames.isEmpty() )
			{
				...
				if ( const auto* assignmentName = normalAssignmentNames.find( m_node.code() ) )
				{
					name = *assignmentName;
				}
				...
				if ( !m_parents.empty() && depth < m_parents.size() )
				{
					for ( size_t i = m_parents.size() - 1; i >= depth; --i )
					{
						...
						if ( const auto* assignmentName = normalAssignmentNames.find( parent.code() ) )
						{
							name = *assignmentName;
							break;
						}
	...
	std::optional<GmodPath> GmodPath::fromShortPath( std::string_view item, const Gmod& gmod, const Locations& locations ) noexcept
	{
	...
	std::optional<GmodPath> GmodPath::fromShortPath( std::string_view item, const Gmod& gmod, const Locations& locations, ParsingErrors& errors ) noexcept
	{
		auto result = internal::fromShortPath( item, gmod, locations );
	...
	std::optional<GmodPath> GmodPath::fromFullPath( std::string_view fullPathStr, const Gmod& gmod, const Locations& locations ) noexcept
	{
	...
	std::optional<GmodPath> GmodPath::fromFullPath( std::string_view fullPathStr, const Gmod& gmod, const Locations& locations, ParsingErrors& errors ) noexcept
	{
		auto result = internal::fromFullPath( fullPathStr, gmod, locations );
