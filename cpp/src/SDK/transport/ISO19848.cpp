...
namespace dnv::vista::sdk::transport
{
	...
	ValidateResult<Value> FormatDataType::validate( std::string_view value ) const
	{
		...
		else if ( nfx::string::iequals( type, "Decimal" ) )
		{
			...
			{
				return ValidateResult<Value>::invalid( { "Invalid decimal value - Value='" + std::string{ value } + "'. " + ex.what() } );
			}
		}
		else if ( nfx::string::iequals( type, "DateTime" ) )
		{
			...
			{
				return ValidateResult<Value>::invalid( { "Invalid datetime value - Value='" + std::string{ value } + "'. Expected format: yyyy-MM-ddTHH:mm:ssZ. " + ex.what() } );
			}
		}
		else
		{
