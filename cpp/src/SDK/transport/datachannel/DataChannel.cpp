...
namespace dnv::vista::sdk::transport::datachannel
{
	...
	NameObject::NameObject(
		std::string namingRule,
		std::optional<CustomNameObjects> customNameObjects )
		: m_namingRule{ std::move( namingRule ) },
	...
	ValidateResult<> Restriction::validateNumber( double number ) const
	{
		...
		{
			errors.push_back(
				"Value " + std::to_string( number ) + " is greater than or equal to " + std::to_string( m_maxExclusive.value() ) );
		}
		...
		{
			errors.push_back(
				"Value " + std::to_string( number ) + " is less than or equal to " + std::to_string( m_minExclusive.value() ) );
		}
	...
	ValidateResult<> Restriction::validateValue( std::string_view value, const Format& format ) const
	{
		...
		else if ( auto i = parsedValue.integer() )
		{
			...
			if ( m_totalDigits )
			{
				...
				{
					return ValidateResult<>::invalid( { "Value has " + std::to_string( numDigits ) + " digits but should be " + std::to_string( m_totalDigits.value() ) } );
				}
			}
		}
		else if ( auto str = parsedValue.string() )
		{
			...
			{
				return ValidateResult<>::invalid( { "Value has length " + std::to_string( length ) + " but should be " + std::to_string( m_length.value() ) } );
			}
			if ( m_maxLength && length > m_maxLength.value() )
			{
				return ValidateResult<>::invalid( { "Value has length " + std::to_string( length ) + " but should be less than " + std::to_string( m_maxLength.value() ) } );
			}
			if ( m_minLength && length < m_minLength.value() )
			{
				return ValidateResult<>::invalid( { "Value has length " + std::to_string( length ) + " but should be greater than " + std::to_string( m_minLength.value() ) } );
			}
			if ( m_pattern )
			{
	...
	Unit::Unit(
		...
		std::optional<CustomElements> customElements )
	...
	Header::Header(
		...
		std::optional<nfx::serialization::json::Document> customHeaders )
