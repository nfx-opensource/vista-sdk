...
namespace dnv::vista::sdk::transport::timeseries
{
		...
		std::function<ValidateResult<>( const DateTimeOffset&, const datachannel::DataChannel&, const Value&, const std::optional<std::string>& )> onEventData ) const
	{
		...
		const bool hasEventData = m_eventData.has_value() && m_eventData->dataSet().has_value() && !m_eventData->dataSet()->empty();
			...
			for ( const auto& table : *m_tabularData )
			{
				...
				for ( std::size_t i = 0; i < dataSets.size(); ++i )
				{
					...
					for ( std::size_t j = 0; j < dataChannelIds.size(); ++j )
					{
						...
						auto formatValidation = dataChannel.property().format().validateValue( dataset.value()[j], parsedValue );

						if ( !formatValidation.isOk() )
						{
							std::string errorMsg = formatValidation.errors.empty() ? "Unknown error" : formatValidation.errors[0];
						...
						{
							std::string errorMsg = customValidation.errors.empty() ? "Unknown error" : customValidation.errors[0];
		...
		if ( hasEventData )
		{
			for ( const auto& eventDataSet : *m_eventData->dataSet() )
			{
				...
				// Validate value format
				Value parsedValue{ Value::String{ "" } };
				auto formatValidation = dataChannel.property().format().validateValue( eventDataSet.value(), parsedValue );
				...
				// Custom validation callback
				auto customValidation = onEventData( eventDataSet.timeStamp(), dataChannel, parsedValue, eventDataSet.quality() );
				if ( !customValidation.isOk() )
				{
					std::string errorMsg = customValidation.errors.empty() ? "Unknown error" : customValidation.errors[0];
					errors.push_back( "DataChannel " + dataChannelId.toString() + " validation failed: " + errorMsg );
