...
namespace dnv::vista::sdk::internal
{
	...
	{
		SmallVector<const GmodNode*, 16> currentParentsBuf;
		SmallVector<const GmodNode*, 16> remainingBuf;
		...
		if ( !path.empty() )
		{
			...
			if ( !prev.isChild( node ) )
			{
				for ( int j = static_cast<int>( path.size() ) - 1; j >= 0; --j )
				{
					...
					std::vector<const GmodNode*> currentVec( currentParentsBuf.begin(), currentParentsBuf.end() );
					std::vector<const GmodNode*> remainingVec;
					const bool pathExists = gmod.pathExistsBetween( currentVec, node, remainingVec );
					if ( pathExists )
					{
						remainingBuf.reserve( remainingVec.size() );
						for ( auto* n : remainingVec )
						{
							remainingBuf.push_back( n );
						}
					}
					if ( !pathExists )
					{
		...
		VisVersion sourceVersion, const GmodPath& sourcePath, VisVersion targetVersion ) const
	{
		...
		std::vector<GmodNode> parentsVec( potentialParents.begin(), potentialParents.end() );
		if ( GmodPath::isValid( parentsVec, *targetEndNode ) )
		{
			return GmodPath{ std::move( parentsVec ), *targetEndNode };
		}
		...
		std::vector<GmodNode> parentsFromPathVec( potentialParentsFromPath.begin(), potentialParentsFromPath.end() );

		LocationSetsVisitor visitor;
		for ( size_t i = 0; i < parentsFromPathVec.size() + 1; ++i )
		{
			const GmodNode& n = ( i < parentsFromPathVec.size() ) ? parentsFromPathVec[i] : targetEndNodeFromPath;
			auto set = visitor.visit( n, i, parentsFromPathVec, targetEndNodeFromPath );
			if ( !set.has_value() )
			{
			...
			{
				if ( j < parentsFromPathVec.size() && location.has_value() )
				{
					parentsFromPathVec[j] = parentsFromPathVec[j].withLocation( location.value() );
				}
				else if ( j >= parentsFromPathVec.size() && location.has_value() )
				{
		...
		if ( !GmodPath::isValid( parentsFromPathVec, targetEndNodeFromPath ) )
		{
			throw std::runtime_error{ "Did not end up with a valid path" };
		}

		return GmodPath{ std::move( parentsFromPathVec ), targetEndNodeFromPath };
	}
} // namespace dnv::vista::sdk::internal
