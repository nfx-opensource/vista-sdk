...
namespace dnv::vista::sdk::internal
{
        ...
        VisVersion sourceVersion, const GmodNode& sourceNode, VisVersion targetVersion, const Gmod& targetGmod ) const
    {
        ...
        // Ultra fast path: if no code change and no location, check if we can reuse source node
        bool hasCodeChange = false;
        std::string_view nextCodeView = sourceNode.code();
        std::string nextCodeStorage;
        ...
        if( versioningNodeOpt.has_value() )
        {
            ...
            if( changeOpt.has_value() )
            {
                ...
                {
                    nextCodeStorage = *change->target;
                    nextCodeView = nextCodeStorage;
                    hasCodeChange = true;
                }
            }
        }

        auto targetNodeOpt = targetGmod.node( nextCodeView );
