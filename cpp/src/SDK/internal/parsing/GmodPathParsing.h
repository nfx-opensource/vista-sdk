...
namespace dnv::vista::sdk::internal
{
	...
	static constexpr const char* ERROR_DIFFERENT_VERSIONS = "Got different VIS versions for Gmod and Locations arguments";
	...
	struct ParseContext
	{
		...
		ParseContext( std::vector<PathNode>&& pathNodeVector, PathNode&& t, std::optional<HashMap<std::string, Location>>&& l,
		...
		const Locations& locations ) noexcept
	{
		...
		for ( const auto partStr : nfx::string::splitView( item, '/' ) )
		{
			...
			if ( dashPos != std::string_view::npos )
			{
				...
				{
					return GmodParsePathResult::Error{ std::string{ ERROR_PREFIX_FAILED_LOCATION } + std::string{ locationPart } };
				}
		...
		std::vector<PathNode> remainingParts( std::make_move_iterator( parts.begin() + 1 ), std::make_move_iterator( parts.end() ) );
