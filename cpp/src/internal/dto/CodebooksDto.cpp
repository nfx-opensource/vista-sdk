...
namespace dnv::vista::sdk
{
	namespace internal
	{
		...
		static std::string_view extractNameHint( const nlohmann::json& json ) noexcept
		{
			try
			{
				const auto nameIt = json.find( dto::codebook::KEY_NAME );
	...
	std::optional<CodebookDto> CodebookDto::tryFromJson( const nlohmann::json& json )
	{
		...
		try
		{
			const auto nameIt = json.find( internal::dto::codebook::KEY_NAME );
			if ( nameIt == json.end() || !nameIt->is_string() )
			{
				auto lease = nfx::string::StringBuilderPool::lease();
				auto builder = lease.builder();
				builder.append( "ERROR: Codebook JSON missing required '" );
				builder.append( internal::dto::codebook::KEY_NAME );
			...
			const auto valuesIt = json.find( internal::dto::codebook::KEY_VALUES );
			if ( valuesIt != json.end() )
			{
				if ( !valuesIt->is_object() )
				{
					...
					builder.append( internal::dto::codebook::KEY_VALUES );
	...
	void from_json( const nlohmann::json& j, CodebookDto& dto )
	{
		...
		const auto nameIt = j.find( internal::dto::codebook::KEY_NAME );
		...
		const auto valuesIt = j.find( internal::dto::codebook::KEY_VALUES );
	...
	void to_json( nlohmann::json& j, const CodebookDto& dto )
	{
		...
		j = nlohmann::json{ { internal::dto::codebook::KEY_NAME, dto.name() },
			{ internal::dto::codebook::KEY_VALUES, dto.values() } };
	}
	...
	std::optional<CodebooksDto> CodebooksDto::tryFromJson( const nlohmann::json& json )
	{
		...
		try
		{
			...
			const auto itemsIt = json.find( internal::dto::codebook::KEY_ITEMS );
			if ( itemsIt != json.end() )
			{
				if ( !itemsIt->is_array() )
				{
					...
					auto builder = lease.builder();
					builder.append( "WARN: '" );
					builder.append( internal::dto::codebook::KEY_ITEMS );
	...
	void from_json( const nlohmann::json& j, CodebooksDto& dto )
	{
		...
		const auto itemsIt = j.find( internal::dto::codebook::KEY_ITEMS );
	...
	void to_json( nlohmann::json& j, const CodebooksDto& dto )
	{
		...
		j = { { internal::dto::KEY_VIS_RELEASE, dto.visVersion() },
			{ internal::dto::codebook::KEY_ITEMS, dto.items() } };
	}
}
