...
namespace dnv::vista::sdk
{
	...
	std::string_view GmodNodeDto::category() const noexcept
	{
		return m_category;
	}

	std::string_view GmodNodeDto::type() const noexcept
	{
		return m_type;
	}

	std::string_view GmodNodeDto::code() const noexcept
	{
		return m_code;
	}

	std::string_view GmodNodeDto::name() const noexcept
	{
		return m_name;
	}
	...
	std::optional<GmodNodeDto> GmodNodeDto::tryFromJson( const nlohmann::json& json )
	{
		...
		try
		{
			...
			if ( nfx::string::isEmpty( tempCode ) )
			{
				auto lease = nfx::string::StringBuilderPool::lease();
				auto builder = lease.builder();
			...
			if ( nfx::string::isEmpty( tempCategory ) )
			{
				auto lease = nfx::string::StringBuilderPool::lease();
				auto builder = lease.builder();
			...
			if ( nfx::string::isEmpty( tempType ) )
			{
				auto lease = nfx::string::StringBuilderPool::lease();
				auto builder = lease.builder();
	...
	GmodDto::GmodDto( std::string visVersion, Items items, Relations relations ) noexcept
	...
	std::string_view GmodDto::visVersion() const noexcept
	{
		return m_visVersion;
	}

	const GmodDto::Items& GmodDto::items() const noexcept
	{
		return m_items;
	}

	const GmodDto::Relations& GmodDto::relations() const noexcept
	{
		return m_relations;
	}
