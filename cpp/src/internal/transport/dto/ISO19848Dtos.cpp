...
namespace dnv::vista::sdk::internal
{
	namespace transport
	{
		...
		static std::string_view extractTypeHint( const nlohmann::json& json ) noexcept
		{
			try
			{
		...
		DataChannelTypeNameDto::DataChannelTypeNameDto( std::string type, std::string description ) noexcept
		...
		std::string_view DataChannelTypeNameDto::type() const noexcept
		{
			return m_type;
		}

		std::string_view DataChannelTypeNameDto::description() const noexcept
		{
		...
		{
			const auto typeIt = j.find( internal::dto::iso19848::KEY_TYPE );
			const auto descIt = j.find( internal::dto::iso19848::KEY_DESCRIPTION );
			...
			if ( typeIt == j.end() || !typeIt->is_string() )
			{
				...
				builder.append( internal::dto::iso19848::KEY_TYPE );
			...
			if ( descIt == j.end() || !descIt->is_string() )
			{
				...
				auto builder = lease.builder();
				builder.append( "DataChannelTypeNameDto JSON missing required '" );
				builder.append( internal::dto::iso19848::KEY_DESCRIPTION );
				builder.append( "' field or not a string" );
		...
		{
			j = nlohmann::json{ { internal::dto::iso19848::KEY_TYPE, dto.m_type }, { internal::dto::iso19848::KEY_DESCRIPTION, dto.m_description } };
		}
		...
		DataChannelTypeNamesDto::DataChannelTypeNamesDto( std::vector<DataChannelTypeNameDto> values ) noexcept
		...
		const std::vector<DataChannelTypeNameDto>& DataChannelTypeNamesDto::values() const noexcept
		{
			return m_values;
		}
		...
		{
			const auto valuesIt = j.find( internal::dto::iso19848::KEY_VALUES );
			if ( valuesIt == j.end() || !valuesIt->is_array() )
			{
				auto lease = nfx::string::StringBuilderPool::lease();
				auto builder = lease.builder();
				builder.append( "DataChannelTypeNamesDto JSON missing required '" );
				builder.append( internal::dto::iso19848::KEY_VALUES );
		...
		void to_json( nlohmann::json& j, const DataChannelTypeNamesDto& dto ) { j = nlohmann::json{ { internal::dto::iso19848::KEY_VALUES, dto.m_values } }; }
		...
		FormatDataTypeDto::FormatDataTypeDto( std::string type, std::string description ) noexcept
			: m_type{ std::move( type ) },
			  m_description{ std::move( description ) }
		{
		}
		...
		std::string_view FormatDataTypeDto::type() const noexcept
		{
			return m_type;
		}

		std::string_view FormatDataTypeDto::description() const noexcept
		{
			return m_description;
		}
		...
		FormatDataTypesDto::FormatDataTypesDto( std::vector<FormatDataTypeDto> values ) noexcept
			: m_values{ std::move( values ) }
		{
		}
		...
		const std::vector<FormatDataTypeDto>& FormatDataTypesDto::values() const noexcept
		{
			return m_values;
		}
		...
		{
			j = nlohmann::json{ { internal::dto::iso19848::KEY_VALUES, dto.m_values } };
		}
	}
}
