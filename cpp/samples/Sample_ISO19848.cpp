...
namespace
{
	std::string wrapText( const std::string& text, size_t width = 100, const std::string& indent = "                         " )
	{
...
int main()
{
	...
	//=====================================================================
	{
		...
		if ( stringResult.isOk() )
		{
			...
			for ( const auto& value : stringValues )
			{
				...
				std::cout << "  '" << value << "'" << std::string( 17 - value.length() + 1, ' ' ) << "-> " << ( result.isOk() ? "Valid" : "Invalid" ) << "\n";
			}
		}
		std::cout << "\n";
	}
		...
		if ( decimalResult.isOk() )
		{
			...
			for ( const auto& tempStr : temperatures )
			{
				try
				{
					...
					std::cout << "  " << std::setw( 8 ) << tempStr << "°C = "
							  << std::fixed << std::setprecision( 1 ) << fahrenheit << "°F\n";
				}
	...
	//=====================================================================
	{
		...
		std::cout << "    string() : " << ( stringValue.string().has_value() ? std::string{ *stringValue.string() } : "nullopt" ) << "\n";

		std::cout << "\n  Integer value:\n";
		std::cout << "    isInteger: " << integerValue.integer().has_value() << "\n";
		std::cout << "    isDecimal: " << integerValue.decimal().has_value() << "\n";
		std::cout << "    integer(): " << ( integerValue.integer().has_value() ? std::to_string( *integerValue.integer() ) : "nullopt" ) << "\n";

		std::cout << "\n  Boolean value:\n";
		std::cout << "    isBoolean: " << booleanValue.boolean().has_value() << "\n";
		std::cout << "    boolean(): " << ( booleanValue.boolean().has_value() ? ( *booleanValue.boolean() ? "true" : "false" ) : "nullopt" ) << "\n";

		std::cout << "\n  Decimal value:\n";
		std::cout << "    isDecimal: " << decimalValue.decimal().has_value() << "\n";
		std::cout << "    decimal(): " << ( decimalValue.decimal().has_value() ? decimalValue.decimal()->toString() : "nullopt" ) << "\n";
