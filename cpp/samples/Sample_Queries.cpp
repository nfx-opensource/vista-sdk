...
int main()
{
	...
	//=====================================================================
	{
		...
		if ( node411Opt.has_value() && nodeC101Opt.has_value() )
		{
			...
			auto query = GmodPathQueryBuilder::empty()
	...
	//=====================================================================
	{
		...
		auto pathQuery = GmodPathQueryBuilder::from( *primaryPath )
							 .withoutLocations()
							 .build();
	...
	//=====================================================================
	{
		...
		if ( node411Opt.has_value() && nodeC101Opt.has_value() )
		{
			...
			// Build path query
			auto pathQuery = GmodPathQueryBuilder::empty()
								 .withNode( *node411, true )
								 .withNode( *nodeC101, true )
								 .build();
			...
			auto localId3 = LocalId::fromString( "/dnv-v2/vis-3-4a/411.1/C101.63/meta/qty-temperature" );
			std::cout << "Test LocalId 3: " << localId3->toString() << "\n";
			std::cout << "  Matches: " << std::boolalpha << query.match( *localId3 ) << " (C101.63 is a subnode of C101)\n";
		}
	...
	//=====================================================================
	{
		...
		auto query1 = LocalIdQueryBuilder::empty()
	...
	//=====================================================================
	{
		...
		if ( node411Opt.has_value() && nodeC101Opt.has_value() )
		{
			...
			const auto baseNodes = GmodPathQueryBuilder::empty()
									   .withNode( *node411, true )
									   .withNode( *nodeC101, true );

			// Build final query using const& copy (demonstrates reusability)
			auto query = LocalIdQueryBuilder::empty()
							 .withPrimaryItem( baseNodes.build() )
							 .build();
