...
int main()
{
	...
	//=====================================================================
	{
		...
		std::vector<timeseries::ConfigurationReference> systemConfigs{
			timeseries::ConfigurationReference{
		...
		std::cout << "  Time Range    : " << header.timeRange()->start().toString() << " - " << header.timeRange()->end().toString() << "\n";
	...
	//=====================================================================
	{
		...
		auto dataConfig = timeseries::ConfigurationReference{
			"DataChannelList.xml",
			DateTimeOffset{ "2024-12-30T00:00:00Z" } };
		...
		auto tabularData = timeseries::TabularData{
			std::vector<timeseries::DataChannelId>{ channelId1, channelId2 },
			dataSets };

		// Create EventData
		auto eventChannelId = timeseries::DataChannelId::fromString( "Alarm1" ).value();
		std::vector<timeseries::EventDataSet> eventSets{
			timeseries::EventDataSet{
		...
		auto timeSeriesData = timeseries::TimeSeriesData{
			dataConfig,
			std::vector<timeseries::TabularData>{ tabularData },
			eventData,
			std::nullopt };
	...
	//=====================================================================
	{
		...
		// Create Header
		auto shipId = ShipId::fromString( "IMO9074729" ).value();
		auto timeRange = timeseries::TimeRange{
			DateTimeOffset{ "2024-12-30T10:00:00Z" },
			DateTimeOffset{ "2024-12-30T14:00:00Z" } };
		...
		// Create Package
		auto package = timeseries::Package{
			header,
	...
	//=====================================================================
	{
		...
		auto tabularData = timeseries::TabularData{
			std::vector<timeseries::DataChannelId>{ channelId },
			std::vector<timeseries::TabularDataSet>{
				timeseries::TabularDataSet{
					DateTimeOffset{ "2024-12-30T10:00:00Z" },
					std::vector<std::string>{ "42.0" },
					std::nullopt } } };
		...
		auto json = nfx::serialization::json::Serializer<timeseries::TimeSeriesDataPackage>::toString( originalPackage, options );
		...
		if ( docOpt.has_value() )
		{
			timeseries::TimeSeriesDataPackage deserializedPackage{
				timeseries::Package{
					std::nullopt,
					std::vector<timeseries::TimeSeriesData>{} } };

			nfx::serialization::json::SerializationTraits<timeseries::TimeSeriesDataPackage>::deserialize( *docOpt, deserializedPackage );
	...
	//=====================================================================
	{
		...
		auto shipId = ShipId::fromString( "IMO9074729" ).value();
		auto configRef = datachannel::ConfigurationReference{
			"DataChannelList.xml",
			DateTimeOffset{ "2024-12-30T00:00:00Z" },
			"v1" };
		...
		auto dataChannelId = datachannel::DataChannelId{
			localId,
			"TempSensor",
			std::nullopt };
		...
		// Create TimeSeriesData
		auto tsDataConfig = timeseries::ConfigurationReference{
			"DataChannelList.xml",
			DateTimeOffset{ "2024-12-30T00:00:00Z" } };
		...
		auto tabularData = timeseries::TabularData{
			...
			std::vector<timeseries::TabularDataSet>{
				timeseries::TabularDataSet{
					DateTimeOffset{ "2024-12-30T10:00:00Z" },
					std::vector<std::string>{ "100.5" },
					std::nullopt } } };

		// Add EventData to trigger onEventData callback
		auto eventData = timeseries::EventData{
			std::vector<timeseries::EventDataSet>{
				timeseries::EventDataSet{
		...
		auto onTabularData = []( const DateTimeOffset& timestamp, const datachannel::DataChannel& channel, const Value& value, const std::optional<std::string>& qualityCode ) {
			...
			if ( channel.property().range() )
				std::cout << "    Range     : [" << channel.property().range()->low() << ", " << channel.property().range()->high() << "]\n";
		...
		auto onEventData = []( const DateTimeOffset& timestamp, const datachannel::DataChannel& channel, const Value& value, const std::optional<std::string>& qualityCode ) {
	...
	//=====================================================================
	{
		...
		auto dataChannelListPackage = datachannel::DataChannelListPackage{
			datachannel::Package{
				datachannel::Header{
					shipId,		  // Ship ID
					configRef,	  // Config reference
		...
		auto timeSeriesData = timeseries::TimeSeriesData{
			timeseries::ConfigurationReference{
				"DataChannelList.xml",
				DateTimeOffset{ "2024-12-30T00:00:00Z" } },
			std::vector<timeseries::TabularData>{
				timeseries::TabularData{
	...
	//=====================================================================
	{
		...
		nfx::serialization::json::Document customHeaders;
		...
		nfx::serialization::json::Document customDataKinds;
		...
		auto json = nfx::serialization::json::Serializer<timeseries::TimeSeriesDataPackage>::toString( tsPackage, options );
