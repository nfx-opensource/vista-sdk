...
void parseLocalId( std::string_view localIdStr )
{
	...
	// Create verbose version for display
	auto verboseLocalId = localId.builder()
	...
	if ( localId.secondaryItem().has_value() )
	{
		...
		// Display secondary node descriptions
		std::cout << "\n  Path breakdown:\n";
...
void queryFile( std::string_view filename, int argc, char* argv[], int startArg )
{
	...
	// Build queries for optional filters
	...
	if ( !primaryPath.empty() )
	{
		...
		pathQuery = GmodPathQueryBuilder::Path::from( *pathOpt )
						.withoutLocations()
						.build();
	}
	...
	if ( !tagName.empty() )
	{
		...
		tagsQuery = MetadataTagsQueryBuilder::empty()
						.withTag( codebookNameEnum, tagValue )
						.build();
	}

	// Build query with chaining
	auto queryBuilder = LocalIdQueryBuilder::empty();
