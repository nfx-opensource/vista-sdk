...
void printHelp()
{
	...
	std::cout << "  --dump-map <file>       Export all nodes to CSV (hash,code,category,type,name,common name,definition,install substructure,normal assignment names)\n\n";
	std::cout << "Examples:\n";
...
void printOverview( const dnv::vista::sdk::Gmod& gmod )
{
	...
	std::sort( sortedChildren.begin(), sortedChildren.end(),
		[]( const GmodNode* a, const GmodNode* b ) {
			return nfx::string::naturalCompare( a->code(), b->code() ) < 0;
		} );
...
void dumpMap( const dnv::vista::sdk::Gmod& gmod, std::string_view filename )
{
	...
	// Write header
	outFile << "HASH,CODE,CATEGORY,TYPE,NAME,COMMON NAME,DEFINITION,COMMON DEFINITION,INSTALL SUBSTRUCTURE,NORMAL ASSIGNMENT NAMES\n";

	size_t count = 0;
	for ( const auto& [code, nodePtr] : sortedEntries )
	{
		uint32_t hashValue = hasher( code );
